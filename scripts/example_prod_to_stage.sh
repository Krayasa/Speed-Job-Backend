...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres job_portal_wagtail -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d job_portal_wagtail -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/job_portal_wagtail/shared/venv/bin/activate && cd /mnt/persist/www/job_portal_wagtail/current/src &&"
